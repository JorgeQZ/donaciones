document.addEventListener("DOMContentLoaded", () => {
      ...
      estados.forEach((nombre) => {
        ...
        if (nombre === valoresMunicipios.ic_entidad) {
          ...
          llenarMunicipios(
            nombre,
            municipioSelect,
            valoresMunicipios.ig_municipio
          );
        }
      });
    })
  ...
  function handleFileChange(inputId, fileLabelId) {
    ...
    input.addEventListener("change", (e) => {
      ...
      if (files.length > 0) {
        ...
        label.textContent =
          names.join(", ") +
          (files.length > max ? " (solo se mostrarán 6)" : "");
  ...
  handleFileChange(
    "inputCertificadoDonaciones",
    "fileNameCertificadoDonaciones"
  );
  ...
  document.querySelectorAll(".custom-select").forEach((select) => {
    ...
    const cssEscape =
      ...
      ((s) => s.replace(/[^a-zA-Z0-9_\-]/g, (c) => `\\${c}`));

    function updateHiddenInput() {
      const wrapper = select.closest(".custom-select-wrapper");
      wrapper
        .querySelectorAll(`input[name="${select.dataset.name}[]"]`)
    ...
    function getQuery() {
      return (inputTag?.value || "").trim().toLowerCase();
    }
    ...
    function refreshOptionsVisibility() {
      ...
      const inputFocused = document.activeElement === inputTag;
      if (optionsBox) {
        optionsBox.style.display =
          inputFocused && visibleCount > 0 ? "block" : "none";
      }
    }

    function allDisabled() {
      return Array.from(options).every((o) => o.classList.contains("disabled"));
    }
    ...
    if (placeholder) {
      placeholder.addEventListener("click", (e) => {
        ...
        const matchingOption = select.querySelector(
          `.custom-options .option[data-value="${cssEscape(value)}"]`
        );
        if (matchingOption) {
          matchingOption.classList.remove("disabled");
        }
    ...
    if (inputTag) {
      ...
      inputTag.addEventListener("keydown", (e) => {
        ...
        const exists = Array.from(placeholder.querySelectorAll(".tag")).some(
          (tag) => tag.dataset.value.toLowerCase() === rawValue.toLowerCase()
        );
        if (exists) {
          inputTag.value = "";
          refreshOptionsVisibility();
...
document.addEventListener("submit", async (e) => {
  ...
  if (!pid || !key || !est || !nonce || !THD_INST || !THD_INST.ajaxUrl) {
    form.submit();
    return;
  }
  ...
  try {
    ...
    if (!json || !json.success)
      throw new Error((json && json.data && json.data.msg) || "Error");
    ...
    if (!document.querySelector(".thd-notice")) {
      ...
      n.innerHTML =
        '<span class="thd-notice__icon">✓</span><div class="thd-notice__text">Estado actualizado</div><button class="thd-notice__close">×</button>';
    ...
    if (btn) {
      btn.disabled = false;
      btn.textContent = btn.dataset._txt || "Listo";
    }
  }
});
